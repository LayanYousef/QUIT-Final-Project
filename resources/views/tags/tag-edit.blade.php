...
<h1 class="underline italic font-bold text-blue-500">تعديل وسم</h1>
</div>
<!--display part-->
...
<form method="POST" action="/tag/{{ $tag->id }}">
  ...
  <div class="space-y-12">
      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <x-form-field class="sm:col-span-4">
          ...
          <div class="mt-2">
            <x-form-input value="{{ $tag->arabic_name }}" name="arabic_name" id="arabic_name"  required/>
        ...
        <x-form-field class="sm:col-span-4">
         <x-form-label for="english_name">العنوان بالإنجليزية</x-form-label>
          <div class="mt-2">
          <x-form-input value="{{ $tag->english_name }}" name="english_name" id="english_name"  required/>
            <!--Error for a single input-->
  ...
  <div class="mt-6 flex items-center justify-end gap-x-6">
    <a href="/tag/{{ $tag->id }}" class="text-sm font-semibold leading-6 text-gray-900 underline">تراجع</a>
    <x-form-button>تعديل</x-form-button>
</form>
  </div>
  </div>
</div>
