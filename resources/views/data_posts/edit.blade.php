...
<h1 class="underline italic font-bold text-blue-500">تعديل منشور</h1>
</div>
<!--display part-->
...
<form method="POST" action="/data_post/{{ $data_post->id }}"><!-- csrf have hidden unique token -->
    @csrf
    @method('PATCH')
  ...
  <div class="space-y-12">
      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <x-form-field class="sm:col-span-4">
          ...
          <div class="mt-2">
            <x-form-input value="{{ $data_post->arabic_title }}" name="arabic_title" id="arabic_title"  required/>
        ...
        <x-form-field class="sm:col-span-4">
         <x-form-label for="english_title">العنوان بالإنجليزية</x-form-label>
          <div class="mt-2">
          <x-form-input value="{{ $data_post->english_title }}" name="english_title" id="english_title"  required/>
            <!--Error for a single input-->
        ...
        <x-form-field class="sm:col-span-4">
         <x-form-label for="league_update">التحديث الدوري</x-form-label>
          <div class="mt-2">
          <x-form-input value="{{ $data_post->league_update }}" name="league_update" id="league_update"  required/>
            <!--Error for a single input-->
        ...
        <x-form-field class="sm:col-span-4">
         <x-form-label for="status">الحالة</x-form-label>
          <div class="mt-2">
          <x-form-input value="{{ $data_post->status }}" name="status" id="status"  required/>
            <!--Error for a single input-->
  ...
  <div class="mt-6 flex items-center justify-end gap-x-6">
    <a href="/data_post/{{ $data_post->id }}" class="text-sm font-semibold leading-6 text-gray-900 underline">تراجع</a>
    <x-form-button type="submit">تعديل</x-form-button>
</form>
  </div>
  </div>
</div>
